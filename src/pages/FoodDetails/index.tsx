...
const FoodDetails: React.FC = () => {
  ...
  useEffect(() => {
    async function loadFood(): Promise<void> {
      ...
      const serializedExtra = response.data.extras.map((itemExtra: Extra) => ({
        ...itemExtra,
        quantity: 0,
      }));

      setExtras(serializedExtra);
  ...
  const cartTotal = useMemo(() => {
    // Calculate cartTotal
    const extraValue = extras.reduce(
      (total, extra) => total + extra.quantity * extra.value,
      0,
    );

    const foodValue = food.price;

    return formatValue((extraValue + foodValue) * foodQuantity);
